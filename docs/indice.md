...
| [`docs/cola-de-trabajo.md`](cola-de-trabajo.md) | Cola de tareas que Claude Code recorre sola, con criterio por tarea y paradas obligatorias. | #entorno #plan | 421 | Al trabajar en modo autónomo | vigente |
| [`docs/auditoria-2026-09-26.md`](auditoria-2026-09-26.md) | Auditoría de seguridad e ingeniería (contrato, app local, sitio y repo): hallazgos verificados, qué está bien y plan de corrección. | #contrato #privacidad #riesgos | 1307 | Antes de corregir o de responder preguntas técnicas del jurado | vigente |
...
| [`docs/diseno-web-movil.md`](diseno-web-movil.md) | Sitio público en el celular: tema dual (Papel y cadena / Cempasúchil) con interruptor, «La app en tu mano», experiencia móvil y «HTML primero». | #marca #demo | 1151 | Antes de diseñar o revisar `web/` | vigente |
...
| [`docs/decisiones.md`](decisiones.md) | Bitácora de decisiones #1–#47 con fecha, motivo y estado; retroalimentación de mentores; lo descartado. | #decisiones | 2,301 | Antes de cambiar cualquier cosa | vigente |
...
| [`spec/2026-09-26_especificacion-web-y-agentes.md`](../spec/2026-09-26_especificacion-web-y-agentes.md) | Sitio público en Vercel (estático, sin llaves) y «Pasillo vivo»: agentes ficticios con las reglas del contrato, repetición verificable de la corrida real, criterios y paradas. | #demo #mapa #plan | 1359 | Antes de trabajar en `web/` | vigente |
| [`spec/2026-09-26_especificacion-gemelo-digital.md`](../spec/2026-09-26_especificacion-gemelo-digital.md) | Vista «Pasillo A-B»: maqueta isométrica donde cada firma real se ve moverse entre bodegas; reglas de privacidad, estética, eventos, datos, criterios y prompt. | #demo #mapa #privacidad | 977 | Antes de construir o revisar la portada de la app | vigente |
...
| [`pitch/2026-09-26_prompt-claude-design-app-movil.md`](../pitch/2026-09-26_prompt-claude-design-app-movil.md) | Prompt para Claude Design: 9 pantallas móviles (cliente y bodega) en tema claro y oscuro, más la lámina web con el teléfono. | #marca #demo #pitch | 871 | Para diseñar la app móvil o el deck | vigente |
...
| [`demo/sembrar.sh`](../demo/sembrar.sh) | Segundo emisor para la demo (spec del frontend §6): Bodega A-73 crea, Doña Mary acepta y Bodega A-73 confirma; una sola vez. Bodega B-40 nunca emite (es la que consulta). Ver decisión #46. | #demo | — | Antes de grabar | vigente |
...
| [`frontend/`](../frontend/) | Portada «Pasillo A-B» (gemelo digital: `vistas/pasillo.js` dibuja la forma real de `plano/pasillo-a-b.json`; `datos/pasillo.js` la carga y trae el esquema genérico de respaldo; eventos en `bitacora.js`) y cuatro vistas en HTML, CSS y JS sin framework: Bodega A-17, Teléfono de Doña Mary, Bodega B-40 y Para el jurado (que también muestra el mapa). | #demo | — | Al tocar la app | vigente |
| [`web/`](../web/) | Sitio público estático para Vercel (#49–#51): Inicio, Pasillo vivo (motor, agentes y vista en `simulacion/`), la demo real verificable (`repeticion.js` con `datos/repeticion.json`, generado solo de la salida real) y Cómo funciona. `gemelo.js` dibuja la copia idéntica de `plano/pasillo-a-b.json`. Sin llaves, sin servidor, sin paquetes; pruebas con `node --test web/pruebas/`. | #demo #mapa | — | Al tocar el sitio público | vigente |
